...
realInput = `.679.....662....71............................805..........862.680...................................................................687....
...
function findCog(s, r, c) {
    if (r < 0 || r > s.length-1 || c < 0 || c > s[r].length-1) {
        return undefined;
    }
    if (s[r][c] === '*') {
        return `${r},${c}`;
...
function isBeginningOfNumber(s, r, c) {
    if (!isNumber(s[r][c])) {
        return false;
    }
    if (c === 0 || !isNumber(s[r][c-1])) {
        return true;
    }
    return false;
}

function isEndOfNumber(s, r, c) {
    if (!isNumber(s[r][c])) {
        return false;
    }
    if (c === s[r].length-1 || !isNumber(s[r][c+1])) {
        return true;
    }
    return false;
}

function findCogAdjacent(s, r, c) {
    return false ||
        findCog(s, r+1, c+0) ||
        findCog(s, r+1, c+1) ||
        findCog(s, r+0, c+1) ||
...
function main(input) {
    ...
    for (let r = 0; r < s.length; r++) {
        for (let c = 0; c < s[r].length; c++) {
            ...
            if (!current && isBeginningOfNumber(s, r, c)) {
                current = {
                    numSoFar: '',
                    nearbyCogKey: '',
                    startR: r,
            ...
            if (current && isNumber(pixel)) {
                current.numSoFar += pixel;
            }

            if (current && !current.nearbyCogKey && findCogAdjacent(s, r, c)) {
                current.nearbyCogKey = findCogAdjacent(s, r, c);
            }

            if (current && isEndOfNumber(s, r, c)) {
                if (current.nearbyCogKey) {
                    const partNum = parseInt(current.numSoFar, 10);
                    if (gears.has(current.nearbyCogKey)) {
                        gears.set(current.nearbyCogKey, [...gears.get(current.nearbyCogKey), partNum]);
                    } else {
                        gears.set(current.nearbyCogKey, [partNum]);
                    }
                }
                current = null;
            }
    ...
    for (let cogKey of gears.keys()) {
        const components = gears.get(cogKey);
