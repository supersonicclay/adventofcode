...
function isSymbol(s, r, c) {
    if (r < 0 || r > s.length-1 || c < 0 || c > s[r].length-1) {
        return false;
    }
    return !isNumber(s[r][c]) && s[r][c] !== '.';
}
...
function isSymbolAdjacent(s, r, c) {
    return false ||
        isSymbol(s, r+1, c+0) ||
        isSymbol(s, r+1, c+1) ||
        isSymbol(s, r+0, c+1) ||
...
function main(input) {
    ...
    for (let r = 0; r < s.length; r++) {
        for (let c = 0; c < s[r].length; c++) {
            ...
            if (current && !current.partNumber && isSymbolAdjacent(s, r, c)) {
                current.partNumber = true;
            }

            if (current && isEndOfNumber(s, r, c)) {
                if (current.partNumber) {
                    result += parseInt(current.numSoFar, 10);
                }
                current = null;
            }
        }
