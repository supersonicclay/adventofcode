...
realInput = `Game 1: 2 green, 12 blue; 6 red, 6 blue; 8 blue, 5 green, 5 red; 5 green, 13 blue; 3 green, 7 red, 10 blue; 13 blue, 8 red
...
function main(input) {
    ...
    for (let line of lines) {
        ...
        const [game, gameInput] = line.split(':');
        const [, id] = game.split(' ');
        const sets = gameInput.split(';').map(set => set.split(', ').map(numColor => {
            const [num, color] = numColor.trim().split(' ');
            return {num: parseInt(num), color};
        }));

        for (let set of sets) {
            for (let {num, color} of set) {
                minPerColor[color] = Math.max(minPerColor[color], num);
            }
        }

        const power = Object.values(minPerColor).reduce((p, c) => p*c, 1);
        result += power;
    }
    console.log(result);
}
