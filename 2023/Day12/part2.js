...
function parse(file) {
    ...
    return input.split('\n').map(line => {
        let [conditionsString, countsString] = line.split(' ');

        conditionsString = `${conditionsString}?${conditionsString}?${conditionsString}?${conditionsString}?${conditionsString}`
        countsString = `${countsString},${countsString},${countsString},${countsString},${countsString}`
...
function possibleArrangements(row) {
    ...
    console.log(`POSSIBLE ARRANGEMENTS: ${conditions.join('')}, ${counts.join(',')} = ${arrangements}`)
